...
public class TeraGen extends Configured implements Tool {
  ...
  static int getDim(JobConf job) {
    return job.getInt("terasort.d", 2);
  }
  
  static void setNumberOfRows(JobConf job, long numRows) {
    job.setLong("terasort.num-rows", numRows);
  }
  static void setDim(JobConf job, int d) {
    job.setInt("terasort.d", d);
  }
...
long rowId = row.get();
  ...
  public int run(String[] args) throws IOException {
    ...
    setDim(job, Integer.parseInt(args[2]));
